...
open class CapturePictureHandler(private val savedDirOrPath: String) : CameraView.Callback {
    ...
    final override fun onPictureCaptured(
        cameraView: CameraView, data: ByteArray, len: Int, width: Int, height: Int, format: Int,
        orientation: Int, facing: Int, timeStampInNs: Long
    ) {
        super.onPictureCaptured(
            cameraView,
            data,
            len,
            width,
        ...
        var picWidth = if (orientation % 90 == 0) height else width
        var picHeight = if (orientation % 90 == 0) width else height
        val i420Bytes = ByteArray(picWidth * picHeight * 3 / 2)
        val success =
            LibYuvUtils.scaleRotationAndMirrorToI420(
                data,
                len,
                i420Bytes,
                width,
    ...
    open fun getCropRect(width: Int, height: Int): Rect = Rect(0, 0, width, height)
}
